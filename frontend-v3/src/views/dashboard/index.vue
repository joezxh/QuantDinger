<template>
  <div class="dashboard-pro" :class="{ 'theme-dark': isDarkTheme }">
    ...
    <div v-if="showSetupGuide && !props.hideSetupGuide" class="setup-guide-card">
    ...
    <div class="table-row">
      ...
      <div class="table-panel">
        ...
        >
          ...
          <template #unrealized_pnl="{ text, record }">
            <div class="pnl-cell">
    ...
    <div class="chart-panel orders-panel">
      <div class="panel-header">
        <div class="panel-title">
          ...
          <a-tooltip :title="soundEnabled ? t('dashboard.clickToMute') : t('dashboard.clickToUnmute')">
            <component
              :is="soundEnabled ? SoundOutlined : AudioMutedOutlined"
              class="sound-toggle"
      ...
      <a-table
        ...
        :pagination="{
          ...
          size: 'small',
          showTotal: (total: number) => t('dashboard.totalOrders', { total })
        }"
        size="small"
      ...
      >
        <template #strategy_name="{ text, record }">
          <div class="symbol-cell">
            <span class="symbol-name">{{ text || '-' }}</span>
        ...
        <template #signal_type="{ text }">
          <span class="type-tag" :class="getTypeClass(text)">
            {{ getSignalTypeText(text) }}
          </span>
        </template>
        <template #exchange="{ text, record }">
          <span
        ...
        <template #notify="{ record }">
          <div class="notify-icons">
            <a-tooltip
              ...
              :title="String(ch)"
            >
              <component :is="getNotifyIconComponent(ch)" class="notify-icon" />
            </a-tooltip>
...
const isDarkTheme = computed(() => {
  return appStore.theme === 'dark' || appStore.theme === 'realdark'
})
...
const summary = ref({
  ...
  strategy_stats: [] as any[],
  daily_pnl_chart: [] as any[],
  strategy_pnl_chart: [] as any[],
  monthly_returns: [] as any[],
  hourly_distribution: [] as any[],
  calendar_months: [] as any[],
  recent_trades: [] as any[],
  current_positions: [] as any[]
})
...
// Sound notification
const orderPollTimer = ref<number | null>(null)
const lastOrderId = ref(0)
const orderPollIntervalMs = 5000
const soundEnabled = ref(true)
const beepCtx = ref<AudioContext | null>(null)

// Graph quality
const graphQuality = ref<any>(null)
...
const strategyStats = computed(() => summary.value.strategy_stats || [])

const showSetupGuide = computed(() => {
  const runningStrategies = Number(summary.value.indicator_strategy_count || 0)
  const hasPositions = Array.isArray(summary.value.current_positions) && summary.value.current_positions.length > 0
  const hasTrades = Array.isArray(summary.value.recent_trades) && summary.value.recent_trades.length > 0
  return runningStrategies === 0 || (!hasPositions && !hasTrades)
})

const maxStrategyPnl = computed(() => {
  const stats = strategyStats.value
  if (!stats.length) return 1
  return Math.max(...stats.map(s => Math.abs(s.total_pnl || 0)), 1)
})

const avgTradesPerDay = computed(() => {
  const chart = summary.value.daily_pnl_chart || []
  const days = chart.length || 1
  const total = performance.value.total_trades || 0
  return (total / days).toFixed(1)
})

const calendarMonths = computed(() => summary.value.calendar_months || [])

const currentCalendarMonth = computed(() => {
  const months = calendarMonths.value
  if (!months.length) return null
  return months[currentCalendarIndex.value] || null
})
...
const orderStrategyFilters = computed(() => {
  const list = Array.isArray(pendingOrders.value) ? pendingOrders.value : []
  const map = new Map()
  ...
  return Array.from(map.values()).sort((a: any, b: any) => String(a.text).localeCompare(String(b.text)))
})
...
const columns = [
  ...
  {
    title: t('dashboard.table.symbol'),
    dataIndex: 'symbol',
    width: 100
  },
  {
  ...
  {
    title: t('dashboard.table.price'),
    dataIndex: 'price',
    customRender: (text: any) => formatNumber(text),
    width: 100
  },
  {
    title: t('dashboard.table.profit'),
    dataIndex: 'profit',
...
const positionColumns = [
  ...
  {
    title: t('dashboard.table.side'),
    dataIndex: 'side',
    slots: { customRender: 'side' }
  },
  {
    title: t('dashboard.table.size'),
    dataIndex: 'size',
    customRender: (text: any) => formatNumber(text, 4)
  },
  {
    title: t('dashboard.table.entryPrice'),
    dataIndex: 'entry_price',
    customRender: (text: any) => formatNumber(text)
  },
  {
    title: t('dashboard.table.pnl'),
    dataIndex: 'unrealized_pnl',
    slots: { customRender: 'unrealized_pnl' },
    align: 'right' as const
  }
]
...
const orderColumns = [
  {
    title: t('dashboard.orderTable.strategy'),
    dataIndex: 'strategy_name',
    slots: { customRender: 'strategy_name' },
    filters: orderStrategyFilters,
    filterMultiple: true,
    onFilter: (value: string, record: any) => String(record && record.strategy_id) === String(value),
    width: 150
  },
  {
    title: t('dashboard.orderTable.exchange'),
    dataIndex: 'exchange_id',
    slots: { customRender: 'exchange' },
    width: 120
  },
  {
    title: t('dashboard.orderTable.notify'),
    dataIndex: 'notify_channels',
    slots: { customRender: 'notify' },
    width: 100
  },
  {
    title: t('dashboard.orderTable.symbol'),
    dataIndex: 'symbol',
    slots: { customRender: 'symbol' },
    width: 110
  },
  {
    title: t('dashboard.orderTable.signalType'),
    dataIndex: 'signal_type',
    slots: { customRender: 'signal_type' },
    width: 100
  },
  {
    title: t('dashboard.orderTable.amount'),
    dataIndex: 'amount',
    slots: { customRender: 'amount' },
    width: 130
  },
  {
    title: t('dashboard.orderTable.price'),
    dataIndex: 'filled_price',
    slots: { customRender: 'price' },
    width: 100
  },
  {
    title: t('dashboard.orderTable.status'),
    dataIndex: 'status',
    slots: { customRender: 'status' },
    width: 130
  },
  {
    title: t('dashboard.orderTable.timeInfo'),
    dataIndex: 'created_at',
    slots: { customRender: 'time_info' },
    width: 160
  }
]
...
function goToStrategyManagement() {
  router.push('/strategy-live')
}

function goToStrategyCreate() {
  router.push({ path: '/strategy-live', query: { mode: 'create' } })
}
...
async function fetchData() {
  try {
    ...
    if ((res as any).code === 1) {
      summary.value = { ...summary.value, ...(res as any).data }
      nextTick(() => {
        initCharts()
      })
    }
...
async function fetchPendingOrders(page?: number, pageSize?: number) {
  ordersLoading.value = true
  try {
    const current = page || ordersPagination.value.current || 1
    const size = pageSize || ordersPagination.value.pageSize || 20
    const res = await getPendingOrders({ page: current, pageSize: size })
    if ((res as any).code === 1) {
      const data = (res as any).data || {}
      pendingOrders.value = data.list || []
      ordersPagination.value.current = Number(data.page || current || 1)
      ordersPagination.value.pageSize = Number(data.pageSize || size || 20)
      ordersPagination.value.total = Number(data.total || 0)
    }
  } catch (e) {
...
async function loadGraphQuality() {
  ...
  try {
    ...
    if ((res as any).code === 1) {
      graphQuality.value = (res as any).data
    }
  } catch (e) {
    // 静默失败，不影响主流程
  } finally {
    graphQualityLoading.value = false
  }
}
...
function playOrderBeep() {
  ...
  try {
    ...
    if (!AudioCtx) return
    if (!beepCtx.value) beepCtx.value = new AudioCtx()
    const ctx = beepCtx.value
    // 部分浏览器需要用户交互后才能播放声音
...
function startOrderPolling() {
  ...
  // 初始化 lastOrderId
  initLastOrderId()
  orderPollTimer.value = window.setInterval(() => {
    pollNewOrders()
  }, orderPollIntervalMs)
}

function stopOrderPolling() {
  if (orderPollTimer.value) {
    clearInterval(orderPollTimer.value)
    orderPollTimer.value = null
  }
}
...
async function initLastOrderId() {
  try {
    ...
    if ((res as any).code === 1 && (res as any).data && (res as any).data.list && (res as any).data.list.length > 0) {
      // 获取最新的订单ID
      lastOrderId.value = (res as any).data.list[0].id || 0
    }
  } catch (e) {
...
async function pollNewOrders() {
  try {
    ...
    if ((res as any).code !== 1 || !(res as any).data || !(res as any).data.list) return

    const orders = (res as any).data.list || []
    if (orders.length === 0) return

    // 检查是否有新订单
    let hasNew = false
    let maxId = lastOrderId.value
    for (const order of orders) {
      const orderId = order.id || 0
      if (orderId > lastOrderId.value) {
        hasNew = true
    ...
    if (hasNew) {
      ...
      // 显示通知
      notification.info({
        message: t('dashboard.newOrderNotify'),
        description: t('dashboard.newOrderDesc'),
        duration: 4
      })
    }
  } catch (e) {
...
function toggleSound() {
  ...
  if (soundEnabled.value) {
    message.success(t('dashboard.soundEnabled'))
  } else {
    message.info(t('dashboard.soundDisabled'))
  }
}

function handleOrdersTableChange(pagination: any) {
  const current = (pagination && pagination.current) ? pagination.current : 1
  const pageSize = (pagination && pagination.pageSize) ? pagination.pageSize : (ordersPagination.value.pageSize || 20)
  ordersPagination.value.current = current
  ordersPagination.value.pageSize = pageSize
  fetchPendingOrders(current, pageSize)
}
...
function getSignalTypeText(type: string) {
  ...
  const typeMap: Record<string, string> = {
    'open_long': t('dashboard.signalType.openLong'),
    'open_short': t('dashboard.signalType.openShort'),
    'close_long': t('dashboard.signalType.closeLong'),
    'close_short': t('dashboard.signalType.closeShort'),
    'add_long': t('dashboard.signalType.addLong'),
    'add_short': t('dashboard.signalType.addShort')
  }
...
function getNotifyIconComponent(channel: string) {
  const c = String(channel || '').trim().toLowerCase()
  const map: Record<string, any> = {
    browser: BellOutlined,
    webhook: LinkOutlined,
    discord: CommentOutlined,
    telegram: MessageOutlined,
    tg: MessageOutlined,
    tele: MessageOutlined,
    email: MailOutlined,
    phone: PhoneOutlined
  }
  return map[c] || NotificationOutlined
}

function formatNumber(num: any, digits = 2) {
...
function formatProfitValue(value: any, record: any) {
...
function prevMonth() {
  if (currentCalendarIndex.value < calendarMonths.value.length - 1) {
...
function getDayProfit(day: number) {
  const m = currentCalendarMonth.value
...
function initPieChart() {
  ...
  const stats = Array.isArray(summary.value.strategy_stats) ? summary.value.strategy_stats : []
  const raw = Array.isArray(summary.value.strategy_pnl_chart) ? summary.value.strategy_pnl_chart : []

  let data: any[] = []
  if (stats.length > 0) {
    data = stats.map((it: any) => {
  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: (p: any) => {
        ...
        const svStr = (sv >= 0 ? '+' : '') + formatNumber(sv, 2)
    ...
    series: [
      {
        ...
        data: data.length > 0 ? data : [{ value: 1, name: t('dashboard.noData'), signedValue: 0 }]
      }
    ]
  }
...
function initDrawdownChart() {
  if (!drawdownChartEl.value) return
  drawdownChart = echarts.init(drawdownChartEl.value)

  const dates = (summary.value.daily_pnl_chart || []).map((item: any) => item.date)
  const values = (summary.value.daily_pnl_chart || []).map((item: any) => Number(item.profit || 0))
  ...
  const option = {
    ...
    tooltip: {
      ...
      axisPointer: {
        type: 'line' as const,
        lineStyle: { color: 'rgba(239, 68, 68, 0.4)', type: 'dashed' as const }
      },
      formatter: (params: any) => {
        const p = Array.isArray(params) ? params[0] : null
        const date = p ? p.axisValue : ''
        const v = p ? Number(p.data || 0) : 0
        const vStr = formatNumber(Math.abs(v), 2)
          ...
          <div style="min-width: 140px;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
              <span style="display:flex;align-items:center;gap:8px;">
                ...
                <span style="color:${textColor}">${t('dashboard.drawdown') || 'Drawdown'}</span>
              </span>
    ...
    xAxis: {
      ...
      axisLabel: {
        color: textColor,
        fontSize: 10,
        formatter: (v: string) => v.slice(5) // Show MM-DD only
      }
    },
    yAxis: {
      type: 'value' as const,
      ...
      splitLine: { lineStyle: { color: gridColor, type: [4, 4] as number[] } },
      max: 0
    },
    series: [
      {
        name: t('dashboard.drawdown') || 'Drawdown',
        type: 'line' as const,
        data: dd,
        smooth: 0.3,
        showSymbol: false,
        ...
        areaStyle: {
          color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
            ...
            { offset: 1, color: 'transparent' as const }
          ])
        },
        markPoint: maxDdValue < 0 ? {
          ...
          label: {
            ...
            fontSize: 10,
            fontWeight: 'bold' as const,
            formatter: () => t('dashboard.label.maxDrawdownPoint') || 'MAX'
          },
          data: [{
            name: t('dashboard.maxDrawdown') || 'Max Drawdown',
        ...
        markLine: {
          ...
          lineStyle: { color: isDark ? '#52525b' : '#a1a1aa', type: 'dashed' as const, width: 1 },
          data: [{ yAxis: 0 }],
          label: { show: false }
        }
      }
...
function initHourlyChart() {
  ...
  const hourlyData = summary.value.hourly_distribution || []
  const hours = hourlyData.map((h: any) => `${String(h.hour).padStart(2, '0')}:00`)
  const counts = hourlyData.map((h: any) => h.count || 0)
  const profits = hourlyData.map((h: any) => h.profit || 0)
  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        let count = 0
        let profit = 0
        const tradeCountLabel = t('dashboard.tradeCount') || 'Trade Count'
        const profitLabel = t('dashboard.profit') || 'Profit'
        const unitLabel = t('dashboard.unit.trades') || ''
        for (const p of arr) {
          if (p.seriesName === tradeCountLabel) count = p.data || 0
          if (p.seriesName === profitLabel) profit = p.data || 0
        }
        const profitColor = profit >= 0 ? '#10b981' : '#ef4444'
        const profitStr = (profit >= 0 ? '+' : '') + formatNumber(profit, 2)
        return `
          <div style="padding: 4px 0;">
    ...
    series: [
      {
        name: t('dashboard.tradeCount') || 'Trade Count',
        type: 'bar' as const,
        data: counts,
        barMaxWidth: 16,
        itemStyle: {
...
// Watch theme changes and reinit charts
