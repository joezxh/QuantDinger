...
export function getUserTimezoneFromStorage(): string {
  try {
    const u = getUserInfo()
    const tz = u?.timezone
 ...
 */
export function formatUserDateTime(input: any, opts: { locale?: string; fallback?: string } = {}): string {
  ...
  try {
    ...
    try {
      ...
      return d.toLocaleString(locale, rest as Intl.DateTimeFormatOptions)
    } catch (e2) {
 ...
 */
export function formatBrowserLocalDateTime(input: any, opts: { locale?: string; fallback?: string } = {}): string {
  const d = parseToDate(input)
  if (!d) return opts.fallback != null ? opts.fallback : '-'
  const locale = opts.locale || (typeof navigator !== 'undefined' && navigator.language) || 'zh-CN'
  const intlOpts: Intl.DateTimeFormatOptions = {
