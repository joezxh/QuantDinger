...
- **AI 辅助市场研究**：覆盖加密货币、美股、外汇、预测市场和跨市场研究流程
- **Python 原生策略开发**：适合量化交易与算法交易团队，支持参数化指标和截面组合策略
- **自动化交易机器人**：网格、马丁、趋势、定投机器人，支持实时监控
- **回测与参数迭代**：适合信号策略、已保存策略、交易机器人和执行假设验证
- **私有化交易基础设施**：适合重视可自托管、本地 LLM 和隐私优先的团队
- **商业化量化产品**：适合需要用户、计费、USDT 支付、积分和后台控制的运营方
...
- **对交易员**：它缩短了从交易想法到交易动作的距离 —— 从 AI 分析到闪电交易，从指标到实盘机器人，全部在同一个界面完成。
- **对量化开发者**：它把 Python 和策略控制权放在核心位置，现已支持参数传递、跨指标调用、截面组合策略和完整的回测历史。
- **对运营方**：它补上了很多开源交易项目缺失的用户、计费、角色和部署能力，包括多用户 RBAC、会员计费、USDT 链上支付和可部署的 Docker Compose 配置。
- **对 AI 工作流**：它让分析结果变得可执行、可复盘、可逐步自动化 —— 并通过 Ollama 本地模型支持实现完全私密的 AI 工作流。
...
| 后端 | Flask API、Python 服务层、策略运行时、ORM 重构后的数据访问层 |
| 存储 | PostgreSQL 16 |
| 缓存 / 后台工作进程支撑 | Redis 7 |
| 交易层 | 多交易所适配、IBKR、MT5 |
...
- 行情通过可插拔数据层拉取。
- 回测在服务端策略引擎中执行，支持策略快照和独立的策略回测持久化。
- 实盘策略与交易机器人（网格、马丁、趋势、定投）由运行时服务生成下单意图。
...
- **网格机器人** — 在配置的价格区间内自动低买高卖
- **马丁机器人** — 分层摊低成本，自动追踪成本基数
- **趋势机器人** — 基于实时权益进行方向性交易和仓位 sizing
- **定投机器人** — 按时间间隔定期投入，与 K 线频率无关

### 指标社区与市场

- 向社区市场上架指标，经过审核与批准流程
- 购买、评分和评论社区指标
- 发布者更新版本后，已购用户可一键同步最新代码
- 绩效追踪：回测收益率、实盘盈亏、胜率
- VIP 会员可免费使用指定指标
